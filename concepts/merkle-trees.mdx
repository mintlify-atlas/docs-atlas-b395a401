...
<Tabs>
  ...
  <Tab title="Checkpoint Sync">
    ...
    ```typescript
    // Download checkpoint (e.g., every 10,000 commitments)
    const checkpoint = await fetch(`https://api.privacycash.com/checkpoints/10000.json`);
    ...
    <Tip>
    Privacy Cash SDK provides built-in checkpoint sync for fast bootstrapping.
    </Tip>
  </Tab>
</Tabs>
